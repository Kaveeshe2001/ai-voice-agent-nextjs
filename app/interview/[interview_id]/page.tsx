...
import { Building, CircleAlert, Clock, Loader2Icon, Settings, Video } from 'lucide-react';
...
const Interview = () => {
  ...
  const GetInterviewDetails = async () => {
    try {
      ...
      console.error(error);
      toast('Failed to load interview details');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="px-10 md:px-28 lg:px-48 xl:px-64 mt-16">
      <div className="flex flex-col gap-2 items-center justify-center border rounded-lg bg-white shadow-md p-10 mb-20 pb-15">
        ...
        <div className="flex flex-col items-center gap-5 mt-5">
          <Image src={'/interviewHeader.png'} alt="interviewHeader" width={500} height={500} className="w-[300px]" />
          ...
          <div className="bg-blue-100 flex flex-col gap-2 rounded-md p-5 w-full mt-3">
            <h2 className="font-bold text-xl text-primary flex gap-2 items-center">
              <CircleAlert className="w-5 h-5" /> Before you begin
            </h2>
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <Button variant="outline" className="cursor-pointer font-bold flex items-center">
              <Settings /> Test Audio & Video
            </Button>
          </div>
        </div>
      </div>
